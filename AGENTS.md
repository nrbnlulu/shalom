...
- /dart/shalom_annotations - a set of annotations used by the dart codegen for declarative graphql operations on widgets (@Query, @Mutation, @Subscription @Fragment)
...
- `@observe` - a graphql directive that the codegen (and the runtime) use to mark that an operation / fragment subscribes to normalized cache updates. This is orthogonal to Flutter: `is_observe` gates the reactive, cache-backed pipeline (`{{Name}}Observable`, `{{Name}}Data.readFrom`/`evictFrom`, cache subscriptions) vs. the older one-shot `Requestable`/`RequestMeta` network-only path that never touches the cache. Plain Dart-only projects consume the same `{{Name}}Observable(...).observe(client)` Stream API directly, no widgets required — Flutter's `*_widget.dart.jinja` templates are just a thin `StatefulWidget` superset wrapping that same API for ergonomics.
...
- flutter widget tests - which verify edge cases regarding to the declarative API, these tests are flutter widget tests and exist in `rust/shalom_dart_codegen/flutter_tests/`
