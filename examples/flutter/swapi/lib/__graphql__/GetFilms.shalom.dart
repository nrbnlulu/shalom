...
// ------------ OBJECT DEFINITIONS -------------
    ...
    static void normalize$inCache(JsonObject data,
        ...
        GetFilms_allFilms.normalize$inCache(
            ...
            this$cached: allFilms$cached is NormalizedObjectRecord ? ctx.shalomContext.getCachedRecord(allFilms$cached.normalizedID()): allFilms$cached,
        ...
        if (allFilms$id != null) {
            final allFilms$normalized = NormalizedObjectRecord(typename: "FilmsConnection", id: allFilms$id);
            this$NormalizedRecord[allFilmsNormalized$Key] = allFilms$normalized;
            
            if (allFilms$cached != null && allFilms$cached is NormalizedObjectRecord && allFilms$cached != allFilms$normalized) {
                ctx.addChangedRecord(allFilms$normalizedID);
            }
        } else {
            this$NormalizedRecord[allFilmsNormalized$Key] = getOrCreateObject(this$NormalizedRecord, allFilmsNormalized$Key);
        }
...
;
    ...
    static (GetFilmsResponse, CacheUpdateContext)  fromResponseImpl(JsonObject data, ShalomCtx ctx){
        ...
        static GetFilmsResponse fromResponse(JsonObject data, {ShalomCtx? ctx}){
            ...
            return fromResponseImpl(data, ctx ?? ShalomCtx.withCapacity()
    ).$1;
        }
    @override
    ...
    static void normalize$inCache(JsonObject data,
            ...
            if (this$normalizedID_temp == null) {
                this$normalizedID = "${parent$normalizedID}.${this$fieldName}";
                    ...
                    final normalized$objRecord = NormalizedObjectRecord(typename: "FilmsConnection", id: this$normalizedID_temp!);
        ...
        GetFilms_allFilms_films.normalize$inCache(
            ...
            this$cached: item$cached is NormalizedObjectRecord ? ctx.shalomContext.getCachedRecord(item$cached.normalizedID()): item$cached,
        ...
        if (films$item$id != null) {
            final films$item$normalized = NormalizedObjectRecord(typename: "Film", id: films$item$id);
            this$NormalizedRecord[item$normalizedKey] = films$item$normalized;
            
            if (item$cached != null && item$cached is NormalizedObjectRecord && item$cached != films$item$normalized) {
                ctx.addChangedRecord(films$normalizedID);
            }
        } else {
            this$NormalizedRecord[item$normalizedKey] = getOrCreateObject(this$NormalizedRecord, item$normalizedKey);
        }
    ...
    static GetFilms_allFilms fromCached(NormalizedRecordData data, ShalomCtx ctx) {
    final films$raw = data["films"];
...
;
    ...
    static void normalize$inCache(JsonObject data,
            ...
            if (this$normalizedID_temp == null) {
                this$normalizedID = "${parent$normalizedID}.${this$fieldName}";
                    ...
                    final normalized$objRecord = NormalizedObjectRecord(typename: "Film", id: this$normalizedID_temp!);
                    if (this$cached != null && this$cached is NormalizedObjectRecord && this$cached as NormalizedObjectRecord != normalized$objRecord){
                        ctx.addChangedRecord("${parent$normalizedID}.${this$fieldName}");
                    }
                    parent$record[this$fieldName] = normalized$objRecord;
    ...
    static GetFilms_allFilms_films fromCached(NormalizedRecordData data, ShalomCtx ctx) {
    final title$raw = data["title"];
...
class RequestGetFilms extends Requestable {
