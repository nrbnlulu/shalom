...
void main() {
  group('HttpLink', () {
    ...
    group('POST requests', () {
      ...
      test('response with null data and errors should resolve to graphql error',
          () async {
        final fakeTransport = FakeTransportLayer(
          responses: [
            {
        ...
        final data = responses[0] as GraphQLError;
        expect(data.errors, isNotNull);
        expect(data.errors.length, 1);
      });
      ...
      test('handles mutation requests', () async {
        ...
        await httpLink.request(request: mutationRequest, headers: []).toList();
